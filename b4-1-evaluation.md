...
- [ ] 상태(`STATE`) 객체를 따로 만들어서 관리한 이유는 무엇이며, 단순 변수로 처리하면 안 되는지 설명할 수 있는가?
- [ ] 반응형 디자인에서 **모바일 퍼스트(Mobile First)** 방식으로 작성한 이유를 이야기할 수 있는가?
